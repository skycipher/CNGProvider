				  ...
				  u32 device_type, u32 data)
{
	...
	write_cmd.address = IXGBE_CPU_TO_BE16((u16)reg_addr);
				 ...
				 u32 device_type, u32 *data)
{
	...
	hic.cmd.address = IXGBE_CPU_TO_BE16((u16)reg_addr);
...
/**
 * ixgbe_set_master_follower_mode - Set up PHY for master/follower mode
 * @hw: pointer to hardware structure
 *
 * Must be called while holding the PHY semaphore and token
 */
STATIC s32 ixgbe_set_master_follower_mode(struct ixgbe_hw *hw)
{
	u16 phy_data;
	s32 rc;

	/* Resolve master/follower mode */
	...
	if (phy_data & IXGBE_M88E1500_1000T_CTRL_MS_ENABLE) {
		if (phy_data & IXGBE_M88E1500_1000T_CTRL_MS_VALUE)
			...
			hw->phy.original_ms_type = ixgbe_ms_force_follower;
	} else {
...
STATIC s32 ixgbe_setup_m88(struct ixgbe_hw *hw)
{
	...
	rc = ixgbe_set_master_follower_mode(hw);
 ...
 **/
#ifdef INTEL_KDNET
_Success_(return == IXGBE_SUCCESS)
#endif
s32 ixgbe_read_ee_hostif_buffer_X550(struct ixgbe_hw *hw,
				     u16 offset, u16 words, /* kdnet */ __out_ecount(words) u16 *data)
{
	struct ixgbe_hic_read_shadow_ram buffer;
	u32 current_word = 0;
	u16 words_to_read;
