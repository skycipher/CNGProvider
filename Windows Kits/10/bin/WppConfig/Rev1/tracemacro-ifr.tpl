...
//
// Generate the WPP_RECORDER_SP_`i.Name` function    2
//
    ...
    )
{
    ...
    WPP_RECORDER( AutoLogContext, level, flags, (LPGUID) traceGuid, id, `i.LogArgs` 0);
}
#endif  // #ifndef WPP_RECORDER_SF_`i.Name`_def
...
#define WPP_CALL_`i.Name`(`i.FixedArgs``i.MacroArgs`) \
     ...
     ((\
       `i.MsgVal`WPP_RECORDER`i.GooId`_FILTER(`i.GooVals`) ? \
        WPP_RECORDER_SF_`i.TypeSig`(WPP_RECORDER`i.GooId`_ARGS(`i.GooVals`), \
                             `i.MsgNo`,WPP_LOCAL_TraceGuids+0`i.MacroExprs`), \
...
#define WPP_CALL_`i.Name`(`i.FixedArgs``i.MacroArgs`) \
     ...
     ((\
       `i.MsgVal`WPP_RECORDER`i.GooId`_FILTER(`i.GooVals`) ? \
        WPP_RECORDER_SF_`i.TypeSig`(WPP_RECORDER`i.GooId`_ARGS(`i.GooVals`), \
                             `i.MsgNo`,WPP_LOCAL_TraceGuids+0`i.MacroExprs`), 1:0\
      ))\
     WPP`i.GooId`_POST(`i.GooVals`)
`ENDFOR`
#endif
...
#define WPP_CALL_`i.Name`(`i.FixedArgs` MSGARGS) \
            ...
            (( \
                `i.MsgVal`WPP_RECORDER`i.GooId`_FILTER(`i.GooVals`) ? \
                WPP_RECORDER_SF_`i.TypeSig`(WPP_RECORDER`i.GooId`_ARGS(`i.GooVals`), \
                                 `i.MsgNo`,WPP_LOCAL_TraceGuids+0 WPP_R`i.ReorderSig` MSGARGS), 1:0  \
             )) \
             WPP`i.GooId`_POST(`i.GooVals`)
`ENDFOR`
#else
