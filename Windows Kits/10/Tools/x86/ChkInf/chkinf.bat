...
rem ** "(" and ")" symbols cause problems in the loop below if they are in the
rem ** PATH variable. Hence we replace them with [leftparen] and [rigthparen]
rem ** respectively and work on the replaced string. Any pathname with a "(" or
rem ** a ")" in it should not contain chkinf.bat, so the PLibPath variable should 
rem ** never point to an invalid location.

set PATH2=%PATH:(=[leftparen]%
set PATH2=%PATH2:)=[rightparen]%
