...
sub ProcInf {
    ...
    # Make sure we've got a version section
    unless ($CurrentINF->sectionDefined("Version"))
    {
        AddErrorRef(1001, 0, "[Version]");
    }
    ...
    ProcSDNames("SOURCEDISKSNAMES.ARM") if ( $CurrentINF->sectionDefined("SourceDisksNames.ARM") );
    ...
    AddErrorRef(1085,$CurrentINF->getIndex("SourceDisksNames.NTARM",0)-1,"ALPHA","SourceDisksNames")
            if ( $CurrentINF->sectionDefined("SourceDisksNames.NTARM") );
    ...
    ProcSDFiles("SOURCEDISKSFILES.ARM") if ( $CurrentINF->sectionDefined("SourceDisksFiles.ARM") );
    ...
    AddErrorRef(1085,$CurrentINF->getIndex("SourceDisksFiles.NTARM",0)-1,"NTARM","SourceDisksFiles")
            if ( $CurrentINF->sectionDefined("SourceDisksFiles.NTARM") );
    ...
    # check to see that sections aren't defined twice
    my $sectionCount;
    foreach ($CurrentINF->getSections())
    {
       $sectionCount = $CurrentINF->count($_);
       # if count > 0 than the section is defined more than once in the INF & included INFs
       if ($sectionCount > 0)
       {
          AddWarnRef(2400, $CurrentINF->getIndex($_)-1, $_);
       }
    }
...
sub ProcManu {
   ...
   foreach $line (@lines) {
        ...
        foreach (@Values) {
            ...
            # $NTARCH NT, NTx86, NTIA64, NTAMD64, NTARM
            # $OSMaj is the OS major version
            # $OSMin is the OS minor version
            # $OSType indicates the type of product, and may be one of the following values (as defined in winnt.h):
            #   VER_NT_WORKSTATION              0x0000001
            #   VER_NT_DOMAIN_CONTROLLER        0x0000002
            #   VER_NT_SERVER                   0x0000003
            # First collate the set of valid install sections
            if ( $CurrentINF->sectionDefined("$Value.$_") ) {
                push(@ModelSections, "$_");
            }
        }

        if ( $CurrentINF->sectionDefined($Value) ) {
            ProcModelSection($Value, IsSelectedModelSection(@ModelSections, $_));
        } else {
        ...
        foreach (@Values) {
            ...
            if ( $CurrentINF->sectionDefined("$Value.$_") ) {
                ProcModelSection("$Value.$_", IsSelectedModelSection(@ModelSections, $_));
            } else {
...
sub ProcCFlagsSection {
    ...
    foreach $line (@lines) {
        ...
        if (uc($Directive) eq "BASICDRIVEROK") {
            AddErrorRef(1530, $CurrentINF->getIndex($Section,$count), $Directive) unless ($INTERNAL);
        }
...
sub ProcClassInstall32Section {
    ...
    if (defined $CheckedSections{uc($Section)} ) {
        ...
        foreach $line (@lines) {
            ...
            $AddProperty++ if (uc($Directive) eq "ADDPROPERTY");
            $count++;
        }
...
sub ProcInstallCoInstallersSection {
    ...
    my($line);
    my($count)=0;
    my($WudfException)=0; # WUDF does not require a copyfiles for the coinstaller since it's inbox
    ...
    if (defined $CheckedSections{uc($Section)})  {
        ...
        foreach $line (@lines) {
           ...
           unless (defined($InstallCoInstallersDirective{uc($Directive)}))  {
            ...
            &{uc($Directive)}($Section, $count, $Value, $WudfException);
                        undef $Globals{CONTEXT};
        ...
        AddErrorRef(1058,$SecStart,"CopyFiles",$Section)
                            unless (($InstallCoInstallersDirective{"COPYFILES"}) || ($WudfException == 1) || $InstallCoInstallersDirective{"NEEDS"});
        $CheckedSections{uc($Section)} = TRUE;
    }
...
#-------------------------------------------------------------------------------------------------
#-- ProcInstallSection($Section, $pnpID, $IsSelectedInstallSection)
sub ProcInstallSection {
   # HKR = HKLM,System\CurrentControlSet\Control\Class\{SetupClassGUID}\{DevInstanceID}
    ...
    my($Section) =  $_[0];
    my $pnpID = $_[1];
    my $IsSelectedInstallSection = $_[2];
    my(@lines)    = $CurrentINF->getSection($Section);
    my($SecStart) = $CurrentINF->getIndex($Section,0)-1;
    my($line);
    my($count)=0;
    my $nonblank=0;
    my($Directive, $Value);
    my($AddReg, $AddProperty, $CopyFiles)  =  (0,0,0);
    ...
    foreach $line (@lines)
    {
        ...
        $Directive=uc($Directive);
         ...
         $AddProperty++ if ($Directive eq "ADDPROPERTY");
         $CopyFiles++ if ($Directive eq "COPYFILES");

        $Globals{CONTEXT} = "DDINSTALL";
        &{uc($Directive)}($Section, $count, $Value, $pnpID, $IsSelectedInstallSection)
    ...
    if ( $CurrentINF->sectionDefined( $Section.".NoResDup" ) ) {
        # Fail and mark this checked.
        AddErrorRef(1606, $CurrentINF->getIndex($Section)-1, ".NoResDup", "Section");
...
#-------------------------------------------------------------------------------------------------
#-- ProcModelSection($Section, $IsSelectedSection)
sub ProcModelSection {
    print("Starting ProcModelSection\n") if ($DEBUG);
    my($Section) =  $_[0];	#Name of the model section
    my($IsSelectedSection) = $_[1];	#Indicates whether this is selected at install time
    my($count)   =     0;
    my($line);
    my(@lines)   = $CurrentINF->getSection($Section);
    ...
    foreach $line (@lines)
    {
        ...
        {
            $pnpID = $Values[1];
            # Check for PnPIds against WDF Samples, could add a check for other IDs here
            CheckWDKPnPId($pnpID);
            ...
            # Verify the undecorated section
            if ($CurrentINF->sectionDefined($Values[0]))
            {
               $OSContext = "All";
               $Globals{"Platform"}  = "X86";
               ProcInstallSection( $Values[0], $pnpID , $IsSelectedSection );
            }
            ...
            if ( $CurrentINF->sectionDefined( $Values[0].".NT") )
            {
                ...
                ProcInstallSection( $Values[0].".NT", $pnpID , $IsSelectedSection );
            }
            ...
            if ( $CurrentINF->sectionDefined( $Values[0].".NTX86") )
            {
                $OSContext = "Windows 2000";
                $Globals{"Platform"}  = "X86";
                ProcInstallSection( $Values[0].".NTX86", $pnpID , $IsSelectedSection );
            }
            ...
            if ( $CurrentINF->sectionDefined( $Values[0].".NTIA64" ) )
            {
                ...
                ProcInstallSection( $Values[0].".NTIA64", $pnpID , $IsSelectedSection );
            }
            ...
            if ( $CurrentINF->sectionDefined( $Values[0].".NTAMD64" ) )
            {
                ...
                ProcInstallSection( $Values[0].".NTAMD64", $pnpID , $IsSelectedSection );
            }

            # Verify explicitly for NT on ARM
            if ( $CurrentINF->sectionDefined( $Values[0].".NTARM" ) )
            {
                $OSContext = "Windows 2000";
                $Globals{"Platform"} = "ARM";
                ProcInstallSection( $Values[0].".NTARM", $pnpID , $IsSelectedSection );
            }
            ...
            if ((!$CurrentINF->sectionDefined($Values[0])) and
                ...
                (!$CurrentINF->sectionDefined($Values[0].".NTAMD64")) and
                (!$CurrentINF->sectionDefined($Values[0].".NTARM")) )
            {
...
#-------------------------------------------------------------------------------------------------#
# Checks against UMDF related development time registry settings and flag it as error in case driver forgets to delete them      #
#-------------------------------------------------------------------------------------------------------- #
sub ProcUmdfAddReg {
    my $DDInstall = shift;
    my @SectionArr;

    my %umdfDevelopmentTimeSettings = ('LOGENABLE' => 1,
                                 'LOGFLAGS' => 1,
                                 'HOSTFAILKDDEBUGBREAK' => 1,
                                 'DEFAULTHOSTPROCESSGUID' => 1,
                                 'NUMDEVICESTACKSMAX' => 1,
                                 'LOGFLUSHPERIODSECONDS' => 1,
                                 'LOGMINIDUMPTYPE' => 1,
                                 'LOGLEVEL' => 1,
                                 'LOGKD' => 1,
                                 'AUTORESTARTDEVICECOUNTLIMIT' => 1,
                                 'LOGSTARTNEWSESSION' => 1);
    my $wudfServiceKey = "SOFTWARE\\MICROSOFT\\WINDOWS NT\\CURRENTVERSION\\WUDF";

    push(@SectionArr, $DDInstall);
    push(@SectionArr, $DDInstall.".HW");
    push(@SectionArr, $DDInstall.".Services");
    push(@SectionArr, $DDInstall.".CoInstallers");

    for (my $i = 0; $i < @SectionArr; $i++ ) {
        my @lines = $CurrentINF->getSection($SectionArr[$i]);
        my ($line, $count);
        my ($directive,$value);
        $count = 0;
        foreach $line (@lines) {
            $count++, next if ($line =~ /^(\s*|;.*)$/);# Skip if it's a blank line
            ($directive,$value) = SplitLine($SectionArr[$i],$count,$line);
                ...
                foreach $regSection (@regSections) {
                    ...
                    if ( $CurrentINF->sectionDefined($regSection)) {
                        ...
                        foreach $regline (@regSectionLines) {
                            ...
                            my @regparts = GetList($regline, $CurrentINF->getIndex($regSection,$regcount));
                            $regcount++, next if ($regparts[1] =~ /^(\s*|;.*)$/);# Skip if it's a blank entry
                            $regparts[1] =~ s/^\"*//; # Strip quotes
                            $regparts[1] =~ s/\"*$//; # Strip quotes
                            if ($wudfServiceKey eq uc($regparts[1])){
                                $regparts[2] =~  s/^\"(.*)\"/$1/;  # Strip the Quotes
                                if (defined($umdfDevelopmentTimeSettings{uc $regparts[2]})) {
                                    AddErrorRef(1519, $CurrentINF->getIndex($regSection, $regcount),$regparts[2])
                                }
                            }
                            $regcount++;
                        }
...
# $KmdfMaxVersion = "Major.Minor"
# $UmdfMaxVersion = "Major.Minor.Service"
#-- ProcWdfCoinstallers($Section, $KmdfDriver, $KmdfMaxVersion, $UmdfDriver, $UmdfMaxVersion, $Win2K) ------------------------------------------------------------------#
sub ProcWdfCoinstallers {
    my $line;
    my $count = 0;
    my $Section = shift;
    my $KmdfDriver = shift;
    my $KmdfMaxVersion = shift;
    my $UmdfDriver = shift;
    my $UmdfMaxVersion = shift;
    my $Win2K = shift;
    my $isModuleWpd = 0;
    my($directive, $value);
                ...
                if ( $CurrentINF->sectionDefined($regSection) ) {
                    ...
                    foreach $regline (@regSectionLines) {
                        ...
                        if (defined($requiredRegDirectives{uc $regparts[2]})) {
                            # If we've already found this directive before, flag it as a warning ????TODOD
                            ...
                            $mCoinstallerLine = $CurrentINF->getIndex($regSection, $regcount);
                            ...
                            if ($installsWdf) {
                                ...
                                $coinstallerLine = $mCoinstallerLine;
    ...
    if ($UmdfDriver) {
        if ($wudfCoinstaller[1] != 1) {
            ...
            if ($wudfVersion =~ /^(\d{2})(\d{3})$/i) {
                my ($wudfRedistMajor, $wudfRedistMinor);
                $wudfRedistMajor = $1 + 0;
                $wudfRedistMinor = $2 + 0;

                if ($UmdfMaxVersion =~ /^(\d*).(\d*)$/) {
                    my ($wudfMaxMajor, $wudfMaxMinor);
                    $wudfMaxMajor = $1 + 0;
                    $wudfMaxMinor = $2 + 0;

                    # Check that the co-installer version is >= of the max version for umdf drivers installed
                    if ($wudfMaxMajor > $wudfRedistMajor || ($wudfMaxMajor == $wudfRedistMajor && $wudfMaxMinor > $wudfRedistMinor)) {
                        AddErrorRef(1518, $coinstallerLine, $wudfRedistMajor.".".$wudfRedistMinor, $UmdfMaxVersion, "UMDF");
                    }
                }
            }
    ...
    if ($KmdfDriver && $wdfVersion =~ /^(\d{2})(\d{3})$/) {
        my ($wdfRedistMajor, $wdfRedistMinor);
        $wdfRedistMajor = $1 + 0;
        $wdfRedistMinor = $2 + 0;
        if ($KmdfMaxVersion =~ /^(\d*).(\d*)$/) {
            my ($wdfMaxMajor, $wdfMaxMinor);
            $wdfMaxMajor = $1 + 0;
            $wdfMaxMinor = $2 + 0;

            if ($wdfMaxMajor > $wdfRedistMajor || ($wdfMaxMajor == $wdfRedistMajor && $wdfMaxMinor > $wdfRedistMinor)) {
                AddErrorRef(1518, $coinstallerLine, $wdfRedistMajor.".".$wdfRedistMinor, $KmdfMaxVersion, "KMDF");
            }
        }
    }
...
sub CheckServiceInstall {
    ...
    my $checkWinUsb = 0;
    my $winUsbControl =2;
    my $expectedDisplayName = "windows driver foundation - user-mode driver framework reflector";
    ...
    my $expectedStartName = "\\driver\\wudfrd";
    my $serviceMatch = 0;
    my ($serviceType, $serviceTypeLine);
    my ($startType, $startTypeLine);
    my ($errorControl, $errorControlLine);
    my ($displayName, $displayNameLine);
    my ($startName, $startNameLine);

	if ($checkWudfRd && lc($ServiceName) ne "wudfrd") {
        $isUniqueWudfRdService = 1;
    }
    ...
    if ($CurrentINF->sectionDefined($Section)) {
        ...
        foreach $line (@lines) {
            ...
            elsif (uc $directive eq 'DISPLAYNAME') {
                $value    =~  s/^\"(.*)\"/$1/;
                $displayName = $value;
                $displayNameLine = $CurrentINF->getIndex($Section, $count);
            }
            elsif (uc $directive eq 'STARTNAME') {
                $value    =~  s/^\"(.*)\"/$1/;
                $startName = $value;
                $startNameLine = $CurrentINF->getIndex($Section, $count);
            }
    ...
    if ($serviceMatch) {
        ...
        # Check that if driver uses unique service name instead of Wudfrd
        # it declares the StartName appropriately from the service-install section
        AddErrorRef(1520, $CurrentINF->getIndex($Section, 0) - 1, "StartName", "\\Driver\\WudfRd", $Section)
            unless (defined($startName) || !$isUniqueWudfRdService);
        ...
        AddErrorRef(1502, $displayNameLine, "DisplayName", $expectedDisplayName)
            unless (lc($displayName) eq $expectedDisplayName || !$checkWudfRd || $isUniqueWudfRdService);

        AddErrorRef(1521, $displayNameLine, $expectedDisplayName)
            unless (lc($displayName) ne $expectedDisplayName || !$checkWudfRd || !$isUniqueWudfRdService);

        AddErrorRef(1502, $startNameLine, "StartName", $expectedStartName)
            unless (lc($startName) eq $expectedStartName || !$checkWudfRd);

        return $ResultFlag;
    }
    elsif ($checkWudfRd && lc($ServiceName) eq "wudfrd") {
        AddErrorRef(1502, $CurrentINF->getIndex($Section, 0) - 1, "ServiceBinary", "\%12\%\\WudfRd.sys");
    }
...
sub CheckWdfSpecificServices {
        ...
        if (uc $directive eq 'ADDSERVICE') {
            ...
            if (scalar @values >= 3)
            {
                ...
                if ($checkWudfRd && !$svcUsesWudfRd) {
                    $resultFlag = CheckServiceInstall($values[2], $wudfRdControl, $values[0]);
                ...
                if ($checkWinUsb && !$svcUsesWinUsb && !$svcIncludeWinUsbInf) { #Check WinUsb Service and Flags
                    $resultFlag = CheckServiceInstall($values[2], $winUsbControl, $values[0]);
            ...
            if (($CurrentINF->sectionDefined($value)) &&
                (uc($value) ne uc($Section))) {

                if (CheckWdfSpecificServices($value, $ControlFlag)) {
                    if ($checkWudfRd) {
                        $svcUsesWudfRd = 1;
                    }
                    if ($checkWinUsb) {
    ...
    if ($checkWudfRd == 1 && $svcUsesWudfRd != 1 && !($svcIncludeClassInf && $svcNeedsClass)) {
        AddErrorRef(1501, $CurrentINF->getIndex($Section, 0) - 1, "WUDFRd");
        AddErrorRef(1520, $CurrentINF->getIndex($Section, 0), "ServiceBinary", "\%12\%\\WudfRd.sys", "service-install");
        $Pass = 0;
    }
...
sub ProcUmdfServiceInstall {
    ...
    foreach $line (@lines) {
        ...
        elsif (uc $directive eq 'UMDFEXTENSIONS') {
            
            my @svcUmdfExtensions= GetList($value, $CurrentINF->getIndex($Section, $count));	

            #Append the list onto the whole list of class extensions
            foreach my $extension (@svcUmdfExtensions){
                $refUmdfExtensions->{uc $extension} = 1;  		
            }
        }
        else {
    ...
    foreach $requiredDirective (keys(%requiredDirectives)) {
        if(($requiredDirective ne 'DRIVERCLSID' and $isUmdf2 == 1) || $isUmdf2 != 1) {
    ...
    if($isUmdf2 == 1 and $requiredDirectives{'DRIVERCLSID'} == 1){
        AddErrorRef(1507, $CurrentINF->getIndex($Section, 0)-1, "DRIVERCLSID declared inside a UMDF 2.0 driver", $Section);
    }
...
#-----------------------------------------------------------------------------------------------------------#
# This function returns the KMDF library version string if the section is valid ("major.minor")
...
sub ProcKmdfServiceInstall {
    ...
    my $count = 0;
    my $line;
    my $kmdfLibraryVersion = "0.0";

    my($directive, $value);
    # If this section has already been processed, return immediately.
    if (defined $CheckedSections{uc($Section)})  {
        return $kmdfLibraryVersion;
    }
...
sub ProcWdfSection {
    ...
    my $umdfKmClientPolicy = "";
    my $umdfFileObjectPolicy = "";
    my $umdfDirectHardwareAccess = "";
    my $umdfRegisterAccessMode = "";
    my $umdfFileObjectPolicyLine = 0;
    my $umdfRegisterAccessModeLine = 0;
    ...
    # Check if WDF section is included from a class driver
    my $svcIncludeClassInf = 0;
    my $svcNeedsClass = 0;
        ...
        if (uc $directive eq 'UMDFSERVICE') {
            ...
            if (scalar @values == 2)
            {
                ...
                if ($CurrentINF->sectionDefined($values[1])) {
                    my $umdfVersion;                   
                    $umdfVersion = ProcUmdfServiceInstall($values[1], \%umdfExtensions);
            ...
            $umdfKmClientPolicy = $value;
        }
        elsif (uc $directive eq 'UMDFFILEOBJECTPOLICY') {
            my %umdfFileObjectPolicyValues = ('REJECTNULLANDUNKNOWNFILEOBJECTS' => 0,
                                              'ALLOWNULLANDUNKNOWNFILEOBJECTS' => 0
                                             );

            AddErrorRef(1509, $CurrentINF->getIndex($Section, $count), $directive)
                unless (defined($umdfFileObjectPolicyValues{uc $value}));
            my $key =$CurrentINF->getIndex($Section, $count);
            $key = $key."\.".$directive;
            $umdfCheckVersions{$key} = "1.11.0";
            $umdfFileObjectPolicy = $value;
            $umdfFileObjectPolicyLine = $CurrentINF->getIndex($Section, $count);
        }
        elsif (uc $directive eq 'UMDFFSCONTEXTUSEPOLICY') {
            my %umdfFsContextPolicyValues = ('CANUSEFSCONTEXT'     => 0,
                                             'CANUSEFSCONTEXT2'    => 0,
                                             'CANNOTUSEFSCONTEXTS' =>0
                                             );

            AddErrorRef(1509, $CurrentINF->getIndex($Section, $count), $directive)
                unless (defined($umdfFsContextPolicyValues{uc $value}));
            my $key =$CurrentINF->getIndex($Section, $count);
            $key = $key."\.".$directive;
            $umdfCheckVersions{$key} = "1.11.0";
        }
        elsif (uc $directive eq 'UMDFHOSTPROCESSSHARING') {
            my %umdfHostProcessSharingValues = ('PROCESSSHARINGDISABLED' => 0,
                                                'PROCESSSHARINGENABLED' => 0
                                               );

            AddErrorRef(1509, $CurrentINF->getIndex($Section, $count), $directive)
                unless (defined($umdfHostProcessSharingValues{uc $value}));
            my $key =$CurrentINF->getIndex($Section, $count);
            $key = $key."\.".$directive;
            $umdfCheckVersions{$key} = "1.11.0";
        }
        elsif (uc $directive eq 'UMDFDIRECTHARDWAREACCESS') {
            my %umdfDirectHardwareAccessValues = ('ALLOWDIRECTHARDWAREACCESS' => 0,
                                                  'REJECTDIRECTHARDWAREACCESS' => 0
                                                 );

            AddErrorRef(1509, $CurrentINF->getIndex($Section, $count), $directive)
                unless (defined($umdfDirectHardwareAccessValues{uc $value}));
            my $key =$CurrentINF->getIndex($Section, $count);
            $key = $key."\.".$directive;
            $umdfCheckVersions{$key} = "1.11.0";
            $umdfDirectHardwareAccess = $value;
        }
        elsif (uc $directive eq 'UMDFREGISTERACCESSMODE') {
            my %umdfRegisterAccessModeValues = ('REGISTERACCESSUSINGSYSTEMCALL' => 0,
                                                'REGISTERACCESSUSINGUSERMODEMAPPING' => 0
                                               );

            AddErrorRef(1509, $CurrentINF->getIndex($Section, $count), $directive)
                unless (defined($umdfRegisterAccessModeValues{uc $value}));
            my $key =$CurrentINF->getIndex($Section, $count);
            $key = $key."\.".$directive;
            $umdfCheckVersions{$key} = "1.11.0";
            $umdfRegisterAccessMode = $value;
            $umdfRegisterAccessModeLine = $CurrentINF->getIndex($Section, $count);
        }
            ...
            if (scalar @values != 2) {
                ...
                if ($CurrentINF->sectionDefined($values[1])) {
                    my $kmdfVersion;
                    $kmdfVersion = ProcKmdfServiceInstall($values[1]);
                    if ($kmdfVersion ne "0.0" && !defined($kmdfLibVersions{$kmdfVersion})) {
                        $kmdfLibVersions{$kmdfVersion} = 1;
                    }
                }
                else {
                    AddErrorRef(1003, $CurrentINF->getIndex($Section, $count), $values[1]);
    ...
    if($isUmdfDriver == 1)
    {
        ...
        # if a service isn't listed for the ServiceOrder directive then
        # check for the same within UmdfExtensions's class extension drivers
        foreach $umdfServiceName (keys(%umdfServiceNames)) {
            if($umdfExtensions{$umdfServiceName} != 1) {
        ...
        # Check if UmdfFileObjectPolicy is specified
        #   this directive is useful only when UmdfKernelModeClientPolicy is specified
        if($umdfFileObjectPolicy ne "" && $umdfKmClientPolicy eq "")
        {
            AddWarnRef(2305, $umdfFileObjectPolicyLine, "UmdfFileObjectPolicy", "UmdfKernelModeClientPolicy");
        }

        if($umdfRegisterAccessMode ne "" && (uc $umdfDirectHardwareAccess) ne "ALLOWDIRECTHARDWAREACCESS")
        {
           AddWarnRef(2305, $umdfRegisterAccessModeLine, "UmdfRegisterAccessMode", "UmdfDirectHardwareAccess=AllowDirectHardwareAccess");
        }
        ...
        my $umdfNumLibVersions = keys %umdfLibVersions;
        # Check for the UMDF versions in case there are any asks for it
        if ($umdfNumRequiredVersions > 0 && $umdfNumLibVersions > 0) {
    ...
    if ($isUmdfDriver == 0 && $isKmdfDriver == 0 && ($svcIncludeClassInf != 1 || $svcNeedsClass != 1)) {
    ...
    if ($CurrentINF->sectionDefined($DDInstall.".CoInstallers")) {
        my $umdfNumLibraryVersions = keys %umdfLibVersions;
        my $umdfMaxVersion = "0.0";
        if ($umdfNumLibraryVersions > 0) {
            $umdfMaxVersion = FindLibraryMaxVersion(\%umdfLibVersions);
        }

        my $kmdfNumLibraryVersions = keys %kmdfLibVersions;
        my $kmdfMaxVersion = "0.0";
        if ($kmdfNumLibraryVersions > 0) {
            $kmdfMaxVersion = FindLibraryMaxVersion(\%kmdfLibVersions);
        }
        ProcWdfCoinstallers($DDInstall.".CoInstallers", $isKmdfDriver, $kmdfMaxVersion, $isUmdfDriver, $umdfMaxVersion, $isWin2K);
    }
    else {
        # Otherwise check if there's any need for Wdf or Wudf Co-installers
        if ($isUmdfDriver) {
            AddWarnRef(2401, $CurrentINF->getIndex($Section, 0) - 1);
        }
...
#------------------------------------------------------------------
# This function returns the maximum version of the framework among the installed WDF drivers
#-- FindLibraryMaxVersion(%WDFLibVersions)
sub FindLibraryMaxVersion {
    my %WdfLibVersions = %{(shift)};
    my $wdfVersion;
    my ($major, $minor);
    my $maxMinor = 0;
    my $maxMajor = 0;

    foreach $wdfVersion (keys %WdfLibVersions) {
        if ($wdfVersion =~ /^(\d*).(\d*)/) {
            $major = $1 + 0; # Make them scalar
            $minor = $2 + 0;
            ...
            elsif ($major == $maxMajor && $minor > $maxMinor) {
                $maxMinor = $minor;
            }
        }
    }
...
sub CheckWDKPnPId {
    ...
    my (%wdkPnpIds) = ("ROOT\\ECHO"                                        => 1,
            ...
            "WDF\\WDFMULTICOMP"                                            => 1,
            "ROOT\\SINGLECOMPONENTFSTATEDEVICE"                            => 1,
            "ROOT\\WUDFVHIDMINI"                                           => 1
            );
...
sub CheckWdfServiceName {
    ...
    my (%kmdfServiceNames) = ("ECHO"                 => 1,
        ...
        "BTHECHOSAMPLECLI"                           => 1,
        "PORTIO"                                     => 1,
        "WUDFBASICDRIVERATL"                         => 1,
        "WPDBASICHARDWARE"                           => 1,
        "WDFMULTICOMP"                               => 1,
        "SINGLECOMPONENTFSTATESVC"                   => 1,
        "WUDFVHIDMINI"                               => 1
    );
...
sub CheckUmdfCLSID{
    ...
    my (%umdfCLSIDs) = ("{d202f373-3aae-45ee-ae73-45766cef14b4}"        => 1,
            ...
            "{522d8dbc-520d-4d7e-8f53-920e5c867e6c}"        => 1,
            );
    ...
    foreach $line (@lines) {
        ...
        AddWarnRef(2033,$CurrentINF->getIndex($Section, $count))
            if (   ((defined($Directive)) and ($Directive =~ /\.inf$/i))
                and not (($INTERNAL) and (uc($ClassMod) eq "PRINTER") and ($Directive =~ /^NTPrint\.inf$/i))); # Internal Printer INFS *can* have NTPrint.INF file listed
...
sub ProcVersion {
    ...
    my(%VerVal)=('SIGNATURE'          , '',   # Should be $CHICAGO$
                 ...
                 'PROVIDER'           , '',   #
                 'CATALOGFILE'        , '',   # used for all non-inbox packages
                 'CATALOGFILE.NT'     , '',   # Optional
                 'CATALOGFILE.NTX86'  , '',   # Optional
                 'CATALOGFILE.NTIA64' , '',   # Optional
                 'CATALOGFILE.NTAMD64', '',   # Optional
                 'CATALOGFILE.NTARM'  , '',   # Optional
                 'DRIVERVER'          , '',   # Must be here or in each model section
                 'DONTREFLECTOFFLINE' , '',   # Optional.  For internal use only.  Should not be present in 3rd party INFs
                 'LAYOUTFILE'         , '',   # must not be used as of Vista
                 'PNPLOCKDOWN'        , '');  # Optional
    ...
    foreach $line (@lines) {
        ...
        if (uc($Directive) eq "DONTREFLECTOFFLINE") {
            AddErrorRef(1530, $CurrentINF->getIndex($Section,$count), $Directive) unless ($INTERNAL);
        }

        if (uc($Directive) eq "LAYOUTFILE") {
           AddErrorRef(1601, $CurrentINF->getIndex($Section,$count), $Directive)
        }

        if (uc($Directive) eq "PNPLOCKDOWN") {
    ...
    # If we aren't looking at a system supplied inf we must have a CatalogFile directive
...
#-------------------------------------------------------------------------------------------------
#-- ADDPROPERTY($Section, $index, $text)
sub ADDPROPERTY
{
   print(STDERR "\tBeginning ADDPROPERTY...\n") if ($DEBUG);
   my $Section  = shift;
   my $Offset   = shift;
   my $linetxt  = shift;
   my $line     = $CurrentINF->getIndex($Section, $Offset);

   print(STDERR "\tValidating '$Section'...\n") if ($DEBUG);

    my(@Values)  = GetList($linetxt,$CurrentINF->getIndex($Section,$Offset));
    my($PropertySection);

    foreach $PropertySection (@Values) {

        if ( $CurrentINF->sectionDefined($PropertySection) ) {
            VerifyAddPropertySection($PropertySection);
        } else {
            AddErrorRef(1003,$CurrentINF->getIndex($Section, $Offset),$PropertySection);
        }
    }
	return(TRUE);
}
...
#-------------------------------------------------------------------------------------------------
#-- VerifyAddPropertySection($Section)
sub VerifyAddPropertySection {
   print("Starting VerifyAddPropertySection\n") if ($DEBUG);
   my $line;
   my $Section  =  shift;
   my @lines    =  $CurrentINF->getSection($Section);
   my $count    =  0;
...
#-------------------------------------------------------------------------------------------------
#-- ADDREG($Section, $index, $text, $exception)
sub ADDREG {
    my $Section  = shift;
    my $Offset   = shift;
    my $linetxt  = shift;
    ...
    foreach $RegSection (@Values) {
        ...
        if ( $CurrentINF->sectionDefined($RegSection) ) {
            VerifyAddRegistrySection($RegSection, $$exception);
        } else {
...
#-------------------------------------------------------------------------------------------------
#-- VerifyAddRegistrySection($Section, $exception)
    ...
    if (defined $CheckedSections{uc($Section)})  {
        ...
        foreach $line (@lines) {
            ...
            if (defined $$exception)  {
                if ($Name =~ /.*CoInstallers32.*$/i) {
                    if ( $Value =~ /.*WudfCoinstaller.*$/i) {
                        # An addreg for WudfCoinstaller is exempted from needing a copyfiles to copy it
                        $$exception = 1;
                    }
                }
            }
            ...
            if (uc $RegRoot eq "HKLM") {
                ...
                if (defined $SubKeys[0] && uc $SubKeys[0] eq "SYSTEM") {
                    ...
                    if (defined $SubKeys[1] && uc $SubKeys[1] eq "CURRENTCONTROLSET") {
                        ...
                        # "HKLM,System\CurrentControlSet\Control\Class" should not have values set but this is not enforceable in Win8.
                        #
                        if (defined $SubKeys[2] && uc $SubKeys[2] eq "HARDWARE PROFILES") {
                            ...
                            AddErrorRef(1603, $CurrentINF->getIndex($Section,$count));
                            ...
                            if (defined $SubKeys[3] && uc $SubKeys[3] eq "CLASS") {
                                # Warning - setting reg value under HKLM,System\CurrentControlSet\Control\Class
                                AddWarnRef(2216, $CurrentINF->getIndex($Section,$count));
                            } elsif (defined $SubKeys[3] && uc $SubKeys[3] eq "CRITICALDEVICEDATABASE") {
                                # Error - setting reg value under HKLM,System\CurrentControlSet\Control\CriticalDeviceDatabase
                                AddErrorRef(1604, $CurrentINF->getIndex($Section,$count));
                            }
                        } elsif (defined $SubKeys[2] && uc $SubKeys[2] eq "HARDWARECONFIG") {
                        	# Error - setting reg value under HKLM,System\HardwareConfig
                        	AddErrorRef(1330, $CurrentINF->getIndex($Section,$count));
                        }
                    } elsif (defined $SubKeys[1] && uc $SubKeys[1] eq "DRIVERDATABASE") {
                        # Error - setting reg value under HKLM,System\DriverDatabase
                        AddErrorRef(1331, $CurrentINF->getIndex($Section,$count));
                    } else {
                        # Error - setting reg value under HKLM,System
                        AddErrorRef(1312, $CurrentINF->getIndex($Section,$count));
                    }
                } elsif (defined $SubKeys[0] && uc $SubKeys[0] eq "DRIVERS") {
                    #
                    # Nothing should set anything in the Drivers hive
                    #
                    AddErrorRef(1332, $CurrentINF->getIndex($Section,$count));
                }
            ...
            if ($SubKey =~ /Windows\\CurrentVersion\\RunOnce/) {
                ...
                AddErrorRef(1605,$CurrentINF->getIndex($Section,$count))
                   unless ($Value =~ /rundll32.+(streamci)/i);
            }
...
sub ADDSERVICE {
    ...
    # 0x1FFF = OR of all possible values for $Flags;
    # 0x1FFF = 0x0000001|0x00000002|0x00000004.... (See DDK Docs for possible values)
    # Allowable flags:
    ...
    #   0x00000400 (SPSVCINST_CLOBBER_SECURITY)
    #   0x00000800 (SPSVCINST_STARTSERVICE)
    #   0x00001000 (SPSVCINST_NOCLOBBER_REQUIREDPRIVILEGES)
...
sub GetSourceDisksFilesList {
    ...
    my @Decorations = (".x86", ".IA64", ".AMD64", ".ARM", "");  # listed in priority order
    my $Decor;
    ...
    if ($#CachedSDFiles > 0) {
        ...
        if ( ! $CurrentINF->sectionDefined("SOURCEDISKSFILES") and
             ...
             ! $CurrentINF->sectionDefined("SOURCEDISKSFILES.AMD64") and
             ! $CurrentINF->sectionDefined("SOURCEDISKSFILES.ARM") ) {
                  AddWarnRef(2111, 0);
        }
    }
        ...
        foreach $line (@lines) {
            ...
            if (($Destination =~ /.sys$/i) and (defined $DestDirs{uc($Section)})){
                AddWarnRef(2035, $CurrentINF->getIndex($Section, $count))
                                       unless (($DestDirs{uc($Section)} =~ /^13/) or
                                               ($DestDirs{uc($Section)} =~ /^12/) or
                                               (($DestDirs{uc($Section)} =~ /^11/) and
                                                ($DestDirs{uc($Section)} =~ /drivers/i)) or
                ...
                if (defined $DestDirs{'DEFAULTDESTDIR'}) {
                    ...
                    AddWarnRef(2034, $CurrentINF->getIndex($Section, $count), $Temp)
                       unless (($DestDirs{'DEFAULTDESTDIR'} =~ /^12/) or
                               ($DestDirs{'DEFAULTDESTDIR'} =~ /^13/));
                } else {
...
sub DEVICEUPGRADEUNSUPPORTED
{
   ...
   AddErrorRef(1116, $CurrentINF->getIndex($Section,$Offset));
   return(TRUE);
}
...
#-------------------------------------------------------------------------------------------------
#-- INCLUDE($Section, $index, $text, $IsSelectedInstallSection)
sub INCLUDE {
    my $Section  = shift;
    my $Offset   = shift;
    my $linetxt  = shift;
    my $pnpid = shift;
    my $IsSelectedInstallSection = shift;
    my $line     = $CurrentINF->getIndex($Section, $Offset);
    ...
    foreach (@files) {
        ...
        } elsif ($IsSelectedInstallSection){
...
sub SYSSETUPPNPFLAGS {
   ...
   AddErrorRef(1118, $CurrentINF->getIndex($Section,$Offset));
...
sub CheckClass {
    ...
    my(%ClassGUIDTable) = (
        ...
        "{C166523C-FE0C-4A94-A586-F1A80CFBBF3E}", "AudioEndpoint",         # No class specific verification
        "{5989fce8-9cd0-467d-8a6a-5419e31529d4}", "AudioProcessingObject", # No class specific verification
        "{C06FF265-AE09-48F0-812C-16753D7CBA83}", "AVC",                   # AVC.pm
        "{72631E54-78A4-11D0-BCF7-00AA00B7B32A}", "BATTERY",               # Battery.pm
        "{53D29EF7-377C-4D14-864B-EB3A85769359}", "BIOMETRIC",             # No class specific verification
        ...
        "{14B62F50-3F15-11DD-AE16-0800200C9A66}", "DigitalMediaDevices",   # No class specific verification
        "{4D36E967-E325-11CE-BFC1-08002BE10318}", "DISKDRIVE",             # DiskDrive.pm
        "{4D36E968-E325-11CE-BFC1-08002BE10318}", "DISPLAY",               # Display.pm
        "{48721B56-6795-11D2-B1A8-0080C72E74A2}", "DOT4",                  # No class specific verification
        "{49CE6AC8-6F86-11D2-B1E5-0080C72E74A2}", "DOT4PRINT",             # No class specific verification
        "{9DA2B80F-F89F-4A49-A5C2-511B085B9E8A}", "EhStorSilo",            # No class specific verification
        "{4D36E969-E325-11CE-BFC1-08002BE10318}", "FDC",                   # FDC.pm
        "{F2E7DD72-6468-4E36-B6F1-6488F42C1B52}", "Firmware",              # No class specific verification (yet)
        "{4D36E980-E325-11CE-BFC1-08002BE10318}", "FLOPPYDISK",            # FloppyDisk.pm
        "{6BDD1FC3-810F-11D0-BEC7-08002BE2092F}", "GPS",                   # No class specific verification
        ...
        "{43675D81-502A-4A82-9F84-B75F418C5DEA}", "Media Center Extender", # No class specific verification
        "{CE5939AE-EBDE-11D0-B181-0000F8753EC4}", "MEDIUMCHANGER",         # MediumChanger.pm
        "{4D36E96C-E325-11CE-BFC1-08002BE10318}", "MEDIA",                 # Media.pm
        "{5099944A-F6B9-4057-A056-8C550228544C}", "Memory",                # No class specific verification
        ...
        "{4658EE7E-F050-11D1-B6BD-00C04FA372A7}", "PNPPRINTERS",           # No class specific verification
        "{4D36E979-E325-11CE-BFC1-08002BE10318}", "PRINTER",               # No class specific verification
        "{50127DC3-0F36-415E-A6CC-4CB3BE910B65}", "PROCESSOR",             # Processor.pm
        "{5630831C-06C9-4856-B327-F5D32586E060}", "Proximity",             # No class specific verification
        "{9D6D66A6-0B0C-4563-9077-A0E9A7955AE4}", "Ramdisk",               # No class specific verification
        "{D48179BE-EC20-11D1-B6B8-00C04FA372A7}", "SBP2",                  # SBP2.pm
        "{4D36E97B-E325-11CE-BFC1-08002BE10318}", "SCSIADAPTER",           # SCSIAdapter.pm
        "{A0A588A4-C46F-4B37-B7EA-C82FE89870C6}", "SDHost",                # No class specific verification
        "{268C95A1-EDFE-11D3-95C3-0010DC4050A5}", "SECURITYACCELERATOR",   # SecurityAccelerator.pm
        "{D94EE5D8-D189-4994-83D2-F68D7D41B0E6}", "SecurityDevices",       # No class specific verification
        "{5175D334-C371-4806-B3BA-71FD53C9258D}", "Sensor",                # No class specific verification
        "{997B5D8D-C442-4F2E-BAF3-9C8E671E9E21}", "SideShow",              # No class specific verification
        "{990A2BD7-E738-46C7-B26F-1CF8FB9F1391}", "SMARTCARD",             # SmartCard
        "{DB4F6DDD-9C0E-45E4-9597-78DBBAD0F412}", "SmartCardFilter",       # No class specific verification
        ...
        "{88A1C342-4539-11D3-B88D-00C04FAD5171}", "TS_Generic",            # No class specific verification
        "{36FC9E60-C465-11CF-8056-444553540000}", "USB",                   # USB.pm
        "{88BAE032-5A81-49F0-BC3D-A4FF138216D6}", "USBDevice",             # No class specific verification
        ...
        "{D61CA365-5AF4-4486-998B-9DB4734C6CA3}", "XnaComposite",          # No class specific verification
        # File system filters
	...
	# table recording classes where the class owners have allowed third parties to use classinstall32 for some
	# reason or another
	my(%ExclusionClassGUIDTable) = (
    ...
    if (defined($ClassGUIDTable{uc($ClassGUID)})) {
        ...
        # System defined classes and their settings should not be modified
       if (!$INTERNAL and not defined($ExclusionClassGUIDTable{uc($ClassGUID)}))
       {
            AddErrorRef(1600, $CurrentINF->getIndex("ClassInstall32",0)-1)
                if ($CurrentINF->sectionDefined('CLASSINSTALL32'));
            AddErrorRef(1600, $CurrentINF->getIndex("ClassInstall32.NT",0)-1)
                if ($CurrentINF->sectionDefined('CLASSINSTALL32.NT'));
            AddErrorRef(1600, $CurrentINF->getIndex("ClassInstall32.NTX86",0)-1)
                if ($CurrentINF->sectionDefined('CLASSINSTALL32.NTX86'));
            AddErrorRef(1600, $CurrentINF->getIndex("ClassInstall32.NTIA64",0)-1)
                if ($CurrentINF->sectionDefined('CLASSINSTALL32.NTIA64'));
            AddErrorRef(1600, $CurrentINF->getIndex("ClassInstall32.NTAMD64",0)-1)
                if ($CurrentINF->sectionDefined('CLASSINSTALL32.NTAMD64'));
            AddErrorRef(1600, $CurrentINF->getIndex("ClassInstall32.NTARM",0)-1)
                if ($CurrentINF->sectionDefined('CLASSINSTALL32.NTARM'));
       }
...
sub GetError {
     ...
     my(%ErrorTable) = (
        1001 => ["This is not a valid INF.  A %s section must be provided."],
        ...
        1005 => ["The ALSOINSTALL directive is not valid and should be removed.  Use Includes/Needs instead."],
        ...
        1116 => ["DeviceUpgradeUnsupported is an unsupported, non-functional directive and should be removed."],
        1118 => ["SysSetupPnpFlags is an unsupported, non-functional directive and should be removed."],
        ...
        1320 => ["Ini2Reg directives are no longer supported in device installation infs and should be removed."],
        ...
        1330 => ["INF files can not set registry entries under 'HKLM,System\\HardwareConfig'."],
        1331 => ["INF files can not set registry entries under 'HKLM,System\\DriverDatabase'."],
        1332 => ["INF files can not set registry entries under 'HKLM,Drivers'."],
        ...
        1511 => ["Service '%s' has not been specified in the UmdfServiceOrder directive, if it's a class extension then use UmdfExtensions directive from the client Umdf service's install section"],
        ...
        1517 => ["Directive: '%s' is supported by only %s version %s and higher."],
        1518 => ["WDF Co-Installer version is %s but INF tries to install a version %s %s driver which needs a higher version of the Co-Installer"],
        1519 => ["UMDF registry value %s can only be used during development time"],
        1520 => ["UMDF driver using a unique service name for WudfRd which is available only for Win8 and above, should declare the %s directive as %s from the %s section"],
        1521 => ["UMDF driver using a unique service name for WudfRd which is available only for Win8 and above, should declare the DisplayName directive different from the WudfRd's original service display name \'%s\'"],


        1530 => ["Directive %s is reserved for internal use only"],

        1600 => ["System defined classes and settings should not be modified."],
        1601 => ["LayoutFile is not supported starting in Windows Vista and should be removed."],
        1603 => ["INF files should not set registry entries under 'HKLM,System\\CurrentControlSet\\Hardware Profiles'."],
        1604 => ["INF files should not set registry entries under 'HKLM,System\\CurrentControlSet\\Control\\CriticalDeviceDatabase'."],
        1605 => ["RunOnce usage for device driver installation is restricted to installation of software-only devices enumerated by SWENUM."],
        1606 => ["%s is no longer a supported %s and should be removed if unnecessary."],
...
sub GetWarning {
    ...
    my(%WarningTable) = (
        ...
        2035 => ["Drivers should be copied to the drivers directory (DirID 12) or run from the driver store (DirID 13). Check your [DestinationDirs] section."],
        ...
        2216 => ["Setting registry entries under 'HKLM,System\\CurrentControlSet\\Control\\Class' may be prohibited in future versions of the OS."],
        ...
        2305 => ["Directive %s is meaningful only when %s is specified"],

        2400 => ["%s is defined more than once.  This will result in the sections being collapsed into a single section which may be unexpected."],
        2401 => ["No UMDF Co-installers are specified. Please confirm this is intentional."],
...
#-------------------------------------------------------------------------------------------------#
# Check for whether this section will be the selected Install section
#-------------------------------------------------------------------------------------------------#
#   IsSelectedModelSection (@ModelSections, $ModelSection)
sub IsSelectedModelSection {
    my(@ModelSections) =  @_;
    my($ModelSection) = pop(@ModelSections);
    my($returnval) = 0;
    my($NTARCH,$OSMaj,$OSMin,$OSType,$Suite) = split(/\./,$ModelSection);
    my($SystemVer) = 0;
    my($SystemArch) = 0;
    my($ProcArch) = 0;
    my($OsMajSelected);
    ...
    $SystemVer = `ver`;
    $SystemArch = `echo %PROCESSOR_ARCHITECTURE%`;

    if ($SystemArch =~ /^*(\w+)/) {
        $ProcArch = $1;
    }

    # Cross-arch is ineligible for selection
    if (!(defined $NTARCH) || $NTARCH =~ /^NTx86/i) {
        if (!($ProcArch =~ /^x86/i)) {
            return 0;
        }
    } elsif ($NTARCH =~ /^NTamd64/i) {
        if (!($ProcArch =~ /^AMD64/i)) {
            return 0;
        }
    } elsif ($NTARCH =~ /^NTarm/i) {
        if (!($ProcArch =~ /^ARM/i)) {
            return 0;
        }
    }

    # Analyze ver output to check the version number
    # Eg. Microsoft Windows [Version 6.2.9200]
    if ($SystemVer =~ /^*.+\[.+(\d)\.(\d)\.(\d\d\d\d)\]/) {
        my $OsMajRunning = $1;
        my $OsMinRunning = $2;
        my $OsBldRunning = $3;
        foreach (@ModelSections) {
            ($NTARCHCurr,$OSMajCurr,$OSMinCurr,$OSTypeCurr,$SuiteCurr) = split(/\./,$_);
            # $NTARCHCurr NT, NTx86, NTIA64, NTAMD64, NTARM
            # $OSMajCurr is the OS major version
            # $OSMinCurr is the OS minor version

            # If the section is cross-arch it is ineligible for install on the machine
            if (!(defined $NTARCHCurr) || $NTARCHCurr =~ /^NTx86/i) {
                if (!($ProcArch =~ /^x86/i)) {
                    next;
                }
            } elsif ($NTARCHCurr =~ /^NTamd64/i) {
                if (!($ProcArch =~ /^AMD64/i)) {
                    next;
                }
            } elsif ($NTARCHCurr =~ /^NTarm/i) {
                if (!($ProcArch =~ /^ARM/i)) {
                    next;
                }
            }

            # Cannot select an OS Version greater if defined
            if (!(defined $OSMajCurr) || ($OSMajCurr <= $OsMajRunning)) {
                #	The minor version must be less than what we're running if defined
                if ((!(defined $OSMajCurr) && !(defined $OMinCurr)) ||
                       ($OSMinCurr <= $OsMinRunning)) {
                    # We have a matching install section
                    # Is it the selected one so far ?
                    if ((!(defined $OsMajSelected) && !(defined $OsMinSelected)) ||
                       (($OSMajCurr > $OsMajSelected) || (($OSMajCurr == $OsMajSelected) && ($OSMinCurr > $OsMinSelected)))) {
                        $OsMajSelected = $OSMajCurr;
                        $OsMinSelected = $OSMinCurr;
                    }
                }
            }
        }

        if ((!(defined $OsMajSelected) && !(defined $OsMinSelected) && !(defined $OsMaj) && !(defined $OsMin)) ||
            (($OSMaj == $OsMajSelected) && (($OSMin == $OsMinSelected)))) {
            $returnval = 1;
        }
    }
