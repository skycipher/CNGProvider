...
package NET;                     # The name of this module. For our purposes,
    ...
    my(%ifTypeBySection); # *IfType value associated with each section

    # Standard *IfType values
    my $IF_TYPE_IEEE80211 = 71;
    my $IF_TYPE_WWANPP = 243;
    my $IF_TYPE_WWANPP2 = 244;
    ...
    4209 => ["\"%s\" is non-localizable hard coded string"],
    4210 => ["Inbox driver must set Provider=Microsoft"],
    4211 => ["Inbox driver must disable flow control by default"],
    4212 => ["ExcludeFromSelect does not = * for Inbox driver. This would allow a user to install drivers for hardware which may not exist in the system"],
    4213 => ["Inbox driver must have non-empty %s section"],
    4214 => ["Inbox driver should not use LayoutFile. Use SourceDisksNames and SourceDisksFiles  instead"],
    4215 => ["\"Include=NetVWiFiBus.INF\" and \"Needs=VWiFiBus.CopyFiles\" not required for non IF_TYPE_IEEE80211/IF_TYPE_WWANPP/IF_TYPE_WWANPP2 IfType drivers"],
    4216 => ["\"Include=NetVWiFiBus.INF\" and \"Needs=VWiFiBus.Services\" not required for non IF_TYPE_IEEE80211/IF_TYPE_WWANPP/IF_TYPE_WWANPP2 IfType drivers"],
    );
    ...
    4005 => ["Unknown interface \"%s\" used in Addreg section"],
    ...
    5003 => ["*IfType Keyword missing from section"],
    5004 => ["*MediaType Keyword missing from section"],
    5005 => ["*PhysicalMediaType Keyword missing from section"],
    5006 => ["Unrecognized value for standardized keyword *IfType"],
    5007 => ["Unrecognized value for standardized keyword *MediaType"],
    5008 => ["Unrecognized value for standardized keyword *PhysicalMediaType"],
    5009 => ["The AddService ServiceName parameter must match the NDI\\Service entry in the Registry."],
    5010 => ["Device name string containing special characters, such as \'\\\' and \'\/\', may not be supported on target platform"],
    5011 => ["IfType IF_TYPE_IEEE80211/IF_TYPE_WWANPP/IF_TYPE_WWANPP2 must also include \"Include=NetVWiFiBus.INF\" and \"Needs=VWiFiBus.CopyFiles\" clauses starting from Windows 7 drivers"],
    5012 => ["IfType IF_TYPE_IEEE80211/IF_TYPE_WWANPP/IF_TYPE_WWANPP2 must also include \"Include=NetVWiFiBus.INF\" and \"Needs=VWiFiBus.Services\" clauses starting from Windows 7 drivers"],
    );
    ...
    my(%ValidInterfaces) = ('ETHERNET' => 1, 'ATM' => 1, 'TOKENRING' => 1, 'SERIAL' => 1, 'FDDI' => 1,
                    ...
                    'STREAMS' => 1, 'WINSOCK' => 1, 'WINNET5' => 1, 'WLAN' => 1, 'NOUPPER' =>1, 'NDISWAN' => 1,
                    'NDISCOWAN' => 1, 'NDIS1394' => 1, 'VWIFI' => 1);
...
sub InfCheck {
    ...
    $NET::Internal = ChkInf::GetGlobalValue("MS_INTERNAL");
...
sub CheckModel($Model) {
    ...
    VerifyModel($Model.".NTAMD64") if ( $inffile->sectionDefined($Model.".NTAMD64") );
    VerifyModel($Model.".NTARM")   if ( $inffile->sectionDefined($Model.".NTARM") );
    ...
    VerifyModel($Model)            if ( $inffile->sectionDefined($Model) && !
                                        ( $inffile->sectionDefined("${Model}.NT") ||
                                          ...
                                          $inffile->sectionDefined("${Model}.NTAMD64") ||
                                          $inffile->sectionDefined("${Model}.NTARM")));
...
#-------------------------------------------------------------------------------------------------#
# CheckDeviceName - Checks device name literal for special characters                             #
#-- CheckDeviceName() ----------------------------------------------------------------------------#
sub CheckDeviceName {
    ...
    foreach $line (@lines)
    {
        ...
        {
            AddNetCardWarning(5010, $inffile->getIndex($section,$count));
        }
...
sub LearnModels {
    ...
    foreach $line (@lines) {
        ...
        $model = @items[0];
        $model =~ s/^\s*//; # Chop leading space
        $model =~ s/\s*$//; # Chop tailing space

        # Get the first string before coma in the $Value, assign it to $Model, and update $Value with the rest of the string
        #if ($Value=~/^\s*([^,\s]*)\s*(,.*)$/)
        #{
        #    $model = $1;
        #    $Value = $2;
        ...
        # Check if device names in $model section are okay
        CheckDeviceName($model);
        ...
        do
        {
            ...
            {
                $ext = $items[$i];
                $ext =~ s/^\s*//; # Chop leading space
                $ext =~ s/\s*$//; # Chop tailing space
                $ext = ".$ext";
            }
            else
            {
            ...
            $sectionName = $model.$ext;

            print(STDERR "\t\tsectionName=$sectionName\n") if ($DEBUG);

            # Check if device names in $sectionName section are okay
            CheckDeviceName($sectionName);

            if ( $inffile->sectionDefined($sectionName) ) {
                # Add the models from $INFSections{$Value} to our running list
                %Models = (%Models, ExpandModelSection($sectionName));
                print(STDERR "\tLearnModels -> AddSection $sectionName\n") if ($DEBUG);
           }
           $count++;
           $i++;
        }while($i <= $#items);
...
sub VerifyModel {
    ...
    my($charlnum);
    my($MillSuffix) = CheckVersionForME("VERSION");
    my($ifType,$mediumType,$physicalMediumType);
    my($netVWiFiBusIncluded,$netVWiFiBusDepencyDefined);
    my($MaxIfTypeValue, $MaxMediumTypeValue, $MaxPhysMediumTypeValue);

    print(STDERR "\tCalling VerifyModel ... $Section $Model\n") if ($DEBUG);

    $ifType = "NotFound";
    $mediumType = "NotFound";
    $physicalMediumType = "NotFound";
    $MaxIfTypeValue = 245;
    $MaxMediumTypeValue = 20;
    $MaxPhysMediumTypeValue = 20;
    $netVWiFiBusIncluded = FALSE;
    $netVWiFiBusDepencyDefined = FALSE;
    $charlnum = 0;
    ...
    foreach $line (@lines) {
        ...
        if (uc($Directive) eq "*IFTYPE")
        {
            if (defined($Values[0]))
            {
                $ifType = $Values[0];
                unless (($ifType >= 0) && ($ifType < $MaxIfTypeValue))
                {
                    AddNetCardWarning(5006, $inffile->getIndex($Section,$count));
                }

                # remember the *IfType defined for this section so that media specific checks
                # can be done in other sections
                $NET::ifTypeBySection{$Section} = $ifType;
            }
        }

        if (uc($Directive) eq "*MEDIATYPE")
        {
            if (defined($Values[0]))
            {
                $mediumType = $Values[0];
                unless (($mediumType >= 0) && ($mediumType < $MaxMediumTypeValue))
                {
                    AddNetCardWarning(5007, $inffile->getIndex($Section,$count));
                }
            }
        }

        if (uc($Directive) eq "*PHYSICALMEDIATYPE")
        {
            if (defined($Values[0]))
            {
                $physicalMediumType = $Values[0];
                unless (($physicalMediumType >= 0) && ($physicalMediumType < $MaxPhysMediumTypeValue))
                {
                    AddNetCardWarning(5008, $inffile->getIndex($Section,$count));
                }
            }
        }
        ...
        if (uc($Directive) eq "INCLUDE")
        {
            if (defined($Values[0]))
            {
                if ($Values[0] =~ /NetVWiFiBus.INF/i)
                {
                    $netVWiFiBusIncluded = TRUE;
                }
            }
        }

        if (uc($Directive) eq "NEEDS")
        {
            if (defined($Values[0]))
            {
                if ($Values[0] =~ /VWiFiBus.CopyFiles/i)
                {
                    $netVWiFiBusDepencyDefined = TRUE;
                }
            }
        }
    ...
    if ($ifType eq "NotFound")
    {
        AddNetCardWarning(5003, $inffile->getIndex($Section));
    }

    if ($ifType eq $IF_TYPE_IEEE80211 || $ifType eq $IF_TYPE_WWANPP || $ifType eq $IF_TYPE_WWANPP2)
    {
        if ($netVWiFiBusIncluded eq FALSE || $netVWiFiBusDepencyDefined eq FALSE)
        {
            # signal a warning instead of an error because there is no straightforward
            # way to know if this section is used for pre-Win7 driver installation as well
            AddNetCardWarning(5011, $inffile->getIndex($Section));
        }
    }
    else
    {
        if ($netVWiFiBusIncluded eq TRUE || $netVWiFiBusDepencyDefined eq TRUE)
        {
            AddNetCardError(4215, $inffile->getIndex($Section));
        }
    }

    if ($mediumType eq "NotFound")
    {
        AddNetCardWarning(5004, $inffile->getIndex($Section));
    }

    if ($physicalMediumType eq "NotFound")
    {
        AddNetCardWarning(5005, $inffile->getIndex($Section));
    }
...
sub CheckBusType {
    ...
    foreach (keys(%AllModels)) {
        ...
        if ($Value eq $Item) {
            ...
            {
                # PCI bustype is also allowed here because netcfg requires it to enable Consistent Device Naming.
                # Some virtual bus enumerated devices will set BusType=5 even though their miniport hardware ID does not contain PCI
                if ($RHS !~ /^1$/ and $RHS !~ /^15$/ and $RHS !~ /^5$/)
                {
                    AddNetCardError(4001,$Line_num);
                }
            }
        ...
        if ($LineItems[2] =~ /SERVICE/i)
        {
            ...
            if ($LineItems[1] =~ /INTERFACES/i)
            {
                ...
                for ($i=0; $i < $cnt; $i++)
                {
                    ...
                    AddNetCardWarning(4005,$inffile->getIndex($Section,$count), $tmp3[$i])
            ...
            if ($LineItems[1] =~ /PARAMS/i)
            {
                ...
                @list = split(',', $line);
                # make sure values for "ParamDesc" and "enum" is not hard coded string
                # if inf file is being checked for Inbox submission
                if ($NET::Internal)
                {
                    if ($LineItems[2] =~ /ParamDesc/i || $LineItems[1] =~ /\\enum\s*/i)
                    {
                        if (@list[$#list] !~ /%[^"]*%/)
                        {
                            AddNetCardError(4209,$inffile->getIndex($Section, $count), @list[$#list]);
                        }
                    }
                }
            }
...
sub CheckServiceName {
    my $Section  = shift;                                #ex- ABC43X_NT61.NTX86.SERVICES
    my $pureSectionName = substr($Section, 0, -9);       #ex- ABC43X_NT61.NTX86
    my $line;
    my @lines    =  $inffile->getSection($Section);
    my $count    = 0;
    my $checkval; # if checkval is non-zero, then we check the associated service name

    my $netVWiFiBusIncluded = FALSE;
    my $netVWiFiBusDepencyDefined = FALSE;
    my $sectionIfType = $NET::ifTypeBySection{$pureSectionName};
    ...
    foreach $line (@lines) {
        ...
        if (uc($Directive) eq "ADDREG")
        {
            ...
            print ("\nucAddReg: ",$ucAddReg,"   ucAddService: ",$ucAddService,"\n");
            ...
            if ($checkval != 0)
            {
                if ((!defined($Values[0])) or (!defined($AddRegList{'SERVICE'})) or ($ucAddService ne $ucAddReg)) {
                    AddNetCardWarning(5009,$inffile->getIndex($Section,$count) );
                }
            }
        }
        ...
        if (uc($Directive) eq "NEEDS")
        {
            if (defined($Values[0]))
            {
                if ($Values[0] =~ /VWiFiBus.Services/i)
                {
                    $netVWiFiBusDepencyDefined = TRUE;
                }
            }
    ...
    if ($sectionIfType eq $IF_TYPE_IEEE80211 || $sectionIfType eq $IF_TYPE_WWANPP || $sectionIfType eq $IF_TYPE_WWANPP2)
    {
        if ($netVWiFiBusIncluded eq FALSE || $netVWiFiBusDepencyDefined eq FALSE)
        {
            # signal a warning instead of an error because there is no straightforward
            # way to know if this section is used for pre-Win7 driver installation as well
            AddNetCardWarning(5012, $inffile->getIndex($Section));
        }
    }
    else
    {
        if ($netVWiFiBusIncluded eq TRUE || $netVWiFiBusDepencyDefined eq TRUE)
        {
            AddNetCardError(4216, $inffile->getIndex($Section));
        }
    }
...
sub CheckVersion
{
    ...
    foreach $line (@lines)
    {
        ...
        if ($Directive eq "Provider")
        {
            # Provider must be microsoft for inbox driver
            if ($Value !~ /Microsoft/)
            {
                AddNetCardError(4210,$inffile->getIndex($Section,$count));
            }

            @items = split('=', $line);

            # make sure no hard coded string is used for Provider directive
            if (@items[$#items] !~ /\%[^"]*\%/)
            {
                AddNetCardError(4209,$inffile->getIndex($Section,$count), @items[$#items]);
            }
        }

        # LayoutFile is not allowed anymore for Inbox driver
        if ($Directive eq "LayoutFile")
        {
            AddNetCardError(4214,$inffile->getIndex($Section,$count));
        }
...
sub CheckControlFlags()
{
    ...
    print(STDERR "\CheckControlFlags ...\n") if ($DEBUG);
    ...
    foreach $line (@lines)
    {
        ...
        if ($Directive eq "ExcludeFromSelect")
        {
            # Must be wildcast for ExcludeFromSelect
            if (!($Value eq "*"))
            {
                AddNetCardError(4212,$inffile->getIndex($Section,$count));
            }
        }
...
#-----------------------CheckSourceDiskSections--------------------------------s
sub CheckSourceDiskSections()
{
    my($Directive, $Value);

    # verify the existence of two sections: SourceDisksNames and SourceDisksFiles
    my @Sections = ("SourceDisksNames", "SourceDisksFiles");
    my $Section;
    my $line;
    my $count;
    my @items;
    ...
    print(STDERR "\CheckSourceDiskSections ...\n") if ($DEBUG);
    ...
    foreach $Section (@Sections)
    {
        ...
        if ($count eq 0)
        {
            if ($nonEmpty)
            {
                AddNetCardError(4213,$inffile->getIndex($Section,$count), $Section);
            }
            else
            {
                AddNetCardError(4213,1, $Section);
            }
        }
    }
}
