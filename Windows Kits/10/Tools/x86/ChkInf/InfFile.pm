...
#---------------------------------------------------------------
    ...
    sub addFile {
        ...
        if (defined($cacheTracker{uc($inf)}))
        {
           return(TRUE);
        }
...
#---------------------------------------------------------------
    ...
    sub _initialize {
        ...
        # short circuit any recursive/circular INF references
        if (!defined($cacheTracker{uc($inf)}))
        {
           $cacheTracker{uc($inf)} = 1;
        }
        else
        {
           return(TRUE);
        ...
        foreach (<INFILE>) {
            ...
            #   inf objects in the persistent cache
                ...
                foreach (@values) {
                    ...
                    # INCLUDEd INFs always need to come from %windir%inf
                    # so the file had better be there or we'll fail
                    if (-e "$ENV{WINDIR}\\inf\\$_")
                    {
                        if (!defined($cacheTracker{uc("$ENV{WINDIR}\\inf\\$_")}))
                        {
