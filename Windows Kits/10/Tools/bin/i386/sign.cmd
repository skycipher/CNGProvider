...
if "%SIGN_NULL%"=="1" (
    if defined CROSS_CERT_ISSUER (
        ...
        set SIGNTOOL_OEM_SIGN=/s my /i "Windows Phone Intermediate 2013" /n "Windows Phone OEM Test Cert 2013 (TEST ONLY)" /fd SHA256
    )
)
...
if "%SIGN_NULL%"=="1" (
    set SIGNTOOL_OEM_SIGN_HAL=/s my /i "Windows Phone Intermediate 2013" /n "Windows Phone OEM HAL Extension Test Cert 2013 (TEST ONLY)" /fd SHA256
)
...
if not "%SIGN_WITH_TIMESTAMP%"=="0" set SIGN_WITH_TIMESTAMP=0
...
if "%ERRORLEVEL%"=="0" (
    ...
    @echo %SIGNTOOL_EXE% : fatal error : Signing failed on %*
)
...
if "%CERTIFICATEFLAG%"=="" set SCENARIO_NAME=WscvWpFileHash

REM Convert from phone scenario name to windows scenario name
if "%CERTIFICATEFLAG%"=="wscv" set SCENARIO_NAME=WscvWpFileHash
...
echo %RazzleToolPath%\urtrun.cmd 4.0 signer.exe authenticode %SIGNER_SIG_TYPE% -s %SCENARIO_NAME% %FILELIST%
%RazzleToolPath%\urtrun.cmd 4.0 signer.exe authenticode %SIGNER_SIG_TYPE% -s %SCENARIO_NAME% %FILELIST%
ENDLOCAL
