...
#define _ACRT_APPLY_TO_LATE_BOUND_MODULES(_APPLY)                                                        \
    ...
    _APPLY(ext_ms_win_kernel32_package_current_l1_1_0,   "ext-ms-win-kernel32-package-current-l1-1-0"  ) \
...
{
    return try_get_MessageBoxA() != nullptr && try_get_MessageBoxW() != nullptr;
}
...
extern "C" bool __cdecl __acrt_is_packaged_app()
{
    enum state { unknown, packaged, not_packaged };

    static long state_cache = unknown;

    if (long const cached_state = __crt_interlocked_read(&state_cache))
    {
        return cached_state == packaged;
    }

    auto const get_current_package_id = try_get_GetCurrentPackageId();
    if (get_current_package_id)
    {
        UINT32 buffer_count = 0;
        if (get_current_package_id(&buffer_count, nullptr) == ERROR_INSUFFICIENT_BUFFER)
        {
            long const cached_state = _InterlockedExchange(&state_cache, packaged);
            if (cached_state != unknown)
            {
                _ASSERTE(cached_state == packaged);
            }
    ...
    long const cached_state = _InterlockedExchange(&state_cache, not_packaged);
    if (cached_state != unknown)
    {
        _ASSERTE(cached_state == not_packaged);
    }
